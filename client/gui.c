...
void onMouseButtonDownEvent(){
    ...
    if ((mx < 200) && (my < 50) && (connectEnabled == 1)) {
        ...
        sendMessageToServer(
            gServerIpAddress, gServerPort, sendBuffer);
        connectEnabled = 0;
    } else if ((mx >= 0) && (mx < 200) && (my >= 90) &&
               (my < 330)) {
        joueurSel = (my - 90) / 60;
        guiltSel = -1;
    } else if ((mx >= 200) && (mx < 680) && (my >= 0) &&
               (my < 90)) {
        objetSel = (mx - 200) / 60;
        guiltSel = -1;
    } else if ((mx >= 100) && (mx < 250) && (my >= 350) &&
               (my < 740)) {
        joueurSel = -1;
        objetSel = -1;
        guiltSel = (my - 350) / 30;
    } else if ((mx >= 250) && (mx < 300) && (my >= 350) &&
               (my < 740)) {
        int ind = (my - 350) / 30;
        ...
        if (guiltSel != -1) {
            ...
            sprintf(
                sendBuffer, "S %d %d %d", gId, joueurSel, objetSel);
...
highlightSelections(SDL_Renderer *renderer) {
        // Player selection
        if (joueurSel != -1) {
...
void drawX(SDL_Renderer *renderer) {
        ...
        for (i = 0; i < 13; i++) {
            if (guiltGuess[i]) {
                SDL_RenderDrawLine(
                    renderer, 250, 350 + i * 30, 300, 380 + i * 30);
                SDL_RenderDrawLine(
                    renderer, 250, 380 + i * 30, 300, 350 + i * 30);
            }
        }
}
...
void drawPlayersName(SDL_Renderer *renderer, TTF_Font *Sans) {
        ...
        for (i = 0; i < 4; i++)
            if (strlen(gNames[i]) > 0) {
                ...
                Message_rect.y =
                    110 + i * 60; // controls the rect's y coordinte
                Message_rect.w =
                    surfaceMessage->w; // controls the width of the rect
                Message_rect.h =
                    surfaceMessage->h; // controls the height of the rect
