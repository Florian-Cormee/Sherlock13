            ...
            sendMessageToClient(tcpClients[id].ipAddress,
              ...
              if (nbClients==4)
              {
                ...
                for (size_t id = 0; id < 4; id++) {
                  // On envoie ses cartes au joueur 'id', ainsi que la ligne qui lui correspond dans tableCartes
                  sprintf(reply, "D %d %d %d", deck[3*id], deck[3*id + 1], deck[3*id+2]);
                  sendMessageToClient(tcpClients[id].ipAddress, tcpClients[id].port,reply);
                  for (size_t j = 0; j < 8; j++) {
                    sprintf(reply, "V %ld %d", j, tableCartes[i][j]);
                    sendMessageToClient(tcpClients[id].ipAddress, tcpClients[id].port, reply);
                  }
                }
